...
- 当前界面对应源码 `72bedee`；正式安装位于 `/Applications/cc-switch-hub.app`。2026-09-15 重新 py2app 打包：同步 `fec2888` 之后的 `reset_source` 预计重置口径与额度刷新 1 分钟间隔，包内 `python39.zip` 编译模块行为、签名、空 HOME 隔离启动均已验证，构建产物与备份已清理。
