...
无边框置顶窄条出现在主屏顶部居中、贴上沿。可拖动（位置记忆、可跨显示器）、右键有「立即刷新 / 开机自启 / 退出」。
...
菜单栏出现用量条：双环 icon（内圈 = 5h、外圈 = 周额度已用比例）+ `58% / 68%` 文字（左侧五小时、右侧周额度）；今日 token、花费和近用模型在点击后的详情中查看。
...
- 拖动改位置（位置记忆、可跨显示器），右键「立即刷新 / 开机自启 / 退出」
- 进度环：填充比例 = 5h 额度水位，颜色按档位（🟢 充足 / 🟡 接近上限 / 🔴 超限 / ⚪ 无数据或过期）
